...
class Data:
    ...
    def load_svmlight(self, filename, gamma=None, docalkernel=False, savefile=None, testfile=None, dobin=False):
        ...
        vf = preprocessing.normalize(data[0].todense(), norm='l2')
        self.load_data(vf, data[1], gamma, docalkernel, savefile, testfile, dobin)

    def load_double_file(self,filename,lblfilename, gamma=None, docalkernel=False, savefile=None, testfile=None, dobin=False):
        # data = load_svmlight_file(filename)
        from sklearn import preprocessing
        vf = np.loadtxt(filename)
        lf = np.loadtxt(lblfilename)
        vf_normalized = preprocessing.normalize(vf, norm='l2')
        # lf_normalized = preprocessing.normalize(lf, norm='l2')
        # print(vf_normalized)
        self.load_data(vf_normalized, lf, gamma, docalkernel, savefile, testfile, dobin)

    def load_list(self,vf,lf, gamma=None, docalkernel=False, savefile=None, testfile=None, dobin=False):
        # data = load_svmlight_file(filename)
        from sklearn import preprocessing
        vf_normalized = preprocessing.normalize(vf, norm='l2')
        # lf_normalized = preprocessing.normalize(lf, norm='l2')
