...
teset = torchvision.datasets.CIFAR10(root='cifar_data',
                                     train=False, download=True, transform=te_transforms)
teset_raw = teset.data
print('Loaded original set')
# print(teset_raw[0])
# print(np.max(teset_raw[0]))
# print(np.min(teset_raw[0]))
...
for num in trange(num_sets):
    num_sel = 2  # use more transformation to introduce dataset diversity
    ...
    np.save('/home/qhu/qhu-data/TS4code/meta_set/cifar_data_2/new_data_' + str(num).zfill(3), new_data)
